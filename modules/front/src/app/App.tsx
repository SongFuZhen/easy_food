...
import { menuItems, menuItemIcons } from "../routing";
          ...
          >
            <Menu mode="inline" style={{ height: "100%", borderRight: 0 }}>
              ...
              {menuItems.map((item, idx) =>
                menuItem(item, "" + (idx + 1 + menuIdx), this.props.intl, idx)
              )}
            </Menu>
          </Layout.Sider>
...
function menuItem(
    ...
    return (
      ...
      >
        {(item as SubMenu).items.map((ri, index) =>
          menuItem(ri, keyString + "-" + (index + 1), intl, index)
        )}
      </Menu.SubMenu>
    );
  }
  ...
  return (
    <Menu.Item key={keyString}>
      <NavLink to={menuLink}>
        <Icon type={menuItemIcons[itemIndex]} />
